...
- 🎯 Entry points to execute your program using `python -m <sygua>` or `$ sygua` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/AdamMusa/sygua/branch/main/graph/badge.svg?token=sygua_token_here)](https://codecov.io/gh/AdamMusa/sygua)
[![CI](https://github.com/AdamMusa/sygua/actions/workflows/main.yml/badge.svg)](https://github.com/AdamMusa/sygua/actions/workflows/main.yml)

Awesome sygua created by AdamMusa

## Install

from source
```bash
git clone https://github.com/AdamMusa/sygua sygua
cd sygua
make install
```
...
```bash
❯ sygua --help
Usage: sygua [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ sygua create-user --help
Usage: sygua create-user [OPTIONS] USERNAME PASSWORD
...
```bash
sygua create-user admin admin --superuser
```
...
Run with `sygua run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `sygua create-user`.
...
-----------------------------------------------------
sygua/__init__.py              4      0   100%
sygua/app.py                  16      1    94%
sygua/cli.py                  21      0   100%
sygua/config.py                5      0   100%
sygua/db.py                   10      0   100%
sygua/models/__init__.py       0      0   100%
sygua/models/content.py       47      1    98%
sygua/routes/__init__.py      11      0   100%
sygua/routes/content.py       52     25    52%
sygua/routes/security.py      15      1    93%
sygua/routes/user.py          52     26    50%
sygua/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from sygua.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in sygua/default.toml.

### As environment variables
```bash
export sygua_KEY=value
export sygua_KEY="@int 42"
export sygua_KEY="@jinja {{ this.db.uri }}"
export sygua_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
sygua_ENV=production sygua run
```
